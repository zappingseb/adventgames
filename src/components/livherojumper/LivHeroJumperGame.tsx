...
import { LEVELS, GRAVITY, JUMP_STRENGTH, COLUMN_X_POSITIONS, RUNNER_WIDTH, RUNNER_HEIGHT, SWIPE_THRESHOLD, SWIPE_VELOCITY_THRESHOLD } from '../../constants/livHeroJumperConstants';
...
const LivHeroJumperGame = forwardRef<{ handleMoveLeft: () => void; handleMoveRight: () => void; handleJump: () => void }, LivHeroJumperGameProps>(({
  level,
  gameActive,
  onScoreChange,
  onLoseLife,
  ...
  const handleTouchEnd = useCallback((e: React.TouchEvent<HTMLCanvasElement>) => {
    ...
    else if (deltaY < -SWIPE_THRESHOLD && velocityY > SWIPE_VELOCITY_THRESHOLD) {
      handleJump();
    }
  ...
  useEffect(() => {
    ...
    const gameLoop = () => {
      ...
      if (shouldSpawn && now - lastGapSpawnRef.current > config.gapFrequency) {
        const gapWidth = config.gapWidthRange.min + 
          Math.random() * (config.gapWidthRange.max - config.gapWidthRange.min);
      ...
      gapsRef.current = gapsRef.current.filter((gap) => gap.y < CANVAS_HEIGHT + 100);
      ...
      COLUMN_X_POSITIONS.forEach((xPos, index) => {
        ...
        ctx.strokeStyle = index === currentColumnRef.current ? '#FFD700' : '#CCCCCC';
        ctx.lineWidth = 2;
        ctx.beginPath();
        ctx.moveTo(x, 0);
      ...
      gapsRef.current.forEach((gap) => {
        ...
        ctx.fillRect(gapColumnX - 30, gap.y, 60, gap.width);
      });

      // Draw runner
      const runnerDrawX = CANVAS_WIDTH * COLUMN_X_POSITIONS[currentColumnRef.current] - RUNNER_WIDTH / 2;
      if (runnerImageRef.current) {
        ...
        ctx.fillRect(runnerDrawX, runnerYRef.current, RUNNER_WIDTH, RUNNER_HEIGHT);
      }
  ...
  }, [gameActive, level, CANVAS_WIDTH, CANVAS_HEIGHT, GROUND_Y, onScoreChange, onLoseLife, onLevelComplete]);
